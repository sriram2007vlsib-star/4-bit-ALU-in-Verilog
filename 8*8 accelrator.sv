...
module till(
  ...
  // -------------------------
  // Latch engine_done, but CLEAR it on new compute start
  // -------------------------
  logic sys_done_r;
  ...
  always_ff @(posedge clk) begin
    if (reset)
      sys_done_r <= 1'b0;
    else if (state == COMPUTE)     // clear when we kick off a new compute
      sys_done_r <= 1'b0;
    else if (engine_done)
      sys_done_r <= 1'b1;
  end
  ...
  // Next-state logic
  // -------------------------
  // Parameters — make these match your top-level
  ...
  always_comb begin
    ...
    case (state)
      ...
      ACCUMULATE: begin
        // After accumulating this k-tile:
        // if more k-tiles remain → load next k-tile
        // else → store result
  ...
  // Clear accumulator only at the START of a new output tile (tile_k == 0)
  // i.e. when we're in LOAD and about to compute the first k-slice
...
module top #(parameter N=4, parameter M=8)(
    ...
    logic [31:0] acc_result[0:N-1][0:N-1];  // ADDED: acc output buffer
